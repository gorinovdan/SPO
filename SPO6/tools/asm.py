...
def parse_asm(path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line_no, raw in enumerate(f, 1):
            ...
            if section in ("const", "data", "meta") or not has_sections:
                if ":" in line:
    ...
    return has_sections, const_defs, data_defs, meta_defs, flat_defs, code_labels, code


def assemble(spec_path, asm_path, out_path):
    _, by_mnemonic, _ = load_spec(spec_path)
    has_sections, const_defs, data_defs, meta_defs, flat_defs, code_labels, code = parse_asm(asm_path)
    ...
    if has_sections:
        ...
        _, _, meta_bytes = build_data_section(meta_defs, asm_path)
    else:
        const_refs = set()
        data_refs = set()
        for inst in code:
