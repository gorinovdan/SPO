...
set "BIN_FILE=%~4"
if "%BIN_FILE%"=="" set "BIN_FILE=%PROJECT_DIR%\results\scheduler_demo.ptptb"
for %%I in ("%BIN_FILE%") do set "BIN_FILE=%%~fI"

set "STDOUT_FILE=%~5"
if "%STDOUT_FILE%"=="" set "STDOUT_FILE=%PROJECT_DIR%\results\scheduler_demo.stdout.txt"
for %%I in ("%STDOUT_FILE%") do set "STDOUT_FILE=%%~fI"

set "TRACE_FILE=%~6"
if "%TRACE_FILE%"=="" set "TRACE_FILE=%PROJECT_DIR%\results\scheduler_demo.trace.txt"
