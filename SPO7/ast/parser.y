...
%token TYPE_KW INTERFACE EXTENDS IMPLEMENTS OVERRIDE
...
var_decl type_def interface_def interface_name_list type_member type_member_set field_def method_def
interface_member interface_member_set
...
type_def
	: TYPE_KW identifier type_member_set END							{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $3);
	}
	| TYPE_KW identifier EXTENDS identifier type_member_set END		{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $4);
		append_to_list_node($$, $5);
	}
	| TYPE_KW identifier IMPLEMENTS interface_name_list type_member_set END	{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $4);
	...
	| TYPE_KW identifier EXTENDS identifier IMPLEMENTS interface_name_list type_member_set END	{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $4);
		append_to_list_node($$, $6);
		append_to_list_node($$, $7);
	}
	| TYPE_KW identifier END											{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
	}
	| TYPE_KW identifier EXTENDS identifier END						{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $4);
	}
	| TYPE_KW identifier IMPLEMENTS interface_name_list END			{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $4);
	}
	| TYPE_KW identifier EXTENDS identifier IMPLEMENTS interface_name_list END	{
		$$ = create_node(AST_TYPE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $4);
...
interface_def
	: INTERFACE identifier interface_member_set END		{
		$$ = create_node(AST_INTERFACE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
		append_to_list_node($$, $3);
	}
	| INTERFACE identifier END							{
		$$ = create_node(AST_INTERFACE_DEF);
		SET_LOC($$, @$);
		append_to_list_node($$, $2);
	}
...
interface_name_list
	: identifier								{ $$ = create_list_node($1); SET_LOC($$, @$); }
	| interface_name_list ',' identifier		{ $$ = append_to_list_node($1, $3); SET_LOC($$, @$); }
	;

type_member_set
	: type_member						{ $$ = create_list_node($1); SET_LOC($$, @$); }
	| type_member_set type_member		{ $$ = append_to_list_node($1, $2); SET_LOC($$, @$); }
	;
...
field_def
	: identifier OF type_ref ';'		{ $$ = create_node(AST_FIELD_DEF); SET_LOC($$, @$); append_to_list_node($$, $1); append_to_list_node($$, $3); }
	;

method_def
	: func_def					{ $$ = $1; }
	| OVERRIDE func_def			{ $$ = $2; SET_LOC($$, @$); $$->op = "override"; }
	;

interface_member_set
	: interface_member						{ $$ = create_list_node($1); SET_LOC($$, @$); }
	| interface_member_set interface_member	{ $$ = append_to_list_node($1, $2); SET_LOC($$, @$); }
	;

interface_member
	: DEF func_signature ';'		{ $$ = $2; SET_LOC($$, @$); $$->op = "abstract"; }
	;
...
var_decl
	: identifier OF type_ref ';'								{ $$ = create_node(AST_VAR_DECL); SET_LOC($$, @$); append_to_list_node($$, $1); append_to_list_node($$, $3); }
	| identifier OF type_ref '=' assignment_expr ';'			{ $$ = create_node(AST_VAR_DECL); SET_LOC($$, @$); append_to_list_node($$, $1); append_to_list_node($$, $3); append_to_list_node($$, $5); }
	;
...
in_block_set
	: statement						{ $$ = create_list_node($1); SET_LOC($$, @$); }
	| func_def						{ $$ = create_list_node($1); SET_LOC($$, @$); }
	| in_block_set statement		{ $$ = append_to_list_node($1, $2); SET_LOC($$, @$); }
	| in_block_set func_def			{ $$ = append_to_list_node($1, $2); SET_LOC($$, @$); }
	;
...
postfix_expr
	...
	| postfix_expr '.' identifier				{ $$ = create_node(AST_MEMBER_ACCESS); SET_LOC($$, @$); append_to_list_node($$, $1); append_to_list_node($$, $3); }
	;
