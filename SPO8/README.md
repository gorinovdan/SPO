...
- `tools/ftp_data_adapter.c` принимает внешний FTP control-порт `127.0.0.1:2121`, пассивный data-порт `127.0.0.1:2020` и последовательно прокидывает команды в FTP-сервер VM; это не сервер Btrfs, не источник файловых данных и не обработчик путей.
...
control-подключения FileZilla принимает C-адаптер, но все пути вида
`/SPO8/report.md` разбирает ASM-код внутри VM.
