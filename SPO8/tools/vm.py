...
def write_mem_byte(mem, addr, value):
    if addr < 0 or addr + 1 > len(mem):
        raise ValueError("memory write out of bounds")
    struct.pack_into("<B", mem, addr, int(value) & 0xFF)
...
def run_vm(binary, spec, spec_by_opcode, max_steps, dump_state_path=None, block_image_path=None):
    ...
    while 0 <= ip < len(code):
        ...
        if mnemonic == "NOP":
            ...
            ip += 1
        elif mnemonic == "INDEXB":
            idx = stack_pop()
            base = stack_pop()
            if not isinstance(base, Address):
                raise ValueError("INDEXB expects address base")
            addr = base.addr + to_int(idx)
            stack_push(Address(addr, base.frame_id))
            ip += 1
        elif mnemonic == "LOADB_IND":
            addr = stack_pop()
            if not isinstance(addr, Address):
                raise ValueError("LOADB_IND expects address")
            addr_frame = frame_for_address(addr)
            stack_push(read_mem_byte(addr_frame.mem, addr.addr))
            ip += 1
        elif mnemonic == "STOREB_IND":
            val = stack_pop()
            addr = stack_pop()
            if not isinstance(addr, Address):
                raise ValueError("STOREB_IND expects address")
            addr_frame = frame_for_address(addr)
            write_mem_byte(addr_frame.mem, addr.addr, val)
            if addr.addr in addr_frame.obj_mem:
                del addr_frame.obj_mem[addr.addr]
            ip += 1
        elif mnemonic == "POP":
            stack_pop()
            ip += 1
